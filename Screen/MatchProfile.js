...
const MatchProfile = (props) => {
        ...
        Auth.currentAuthenticatedUser()
            .then(async(data) => {
                if(props&&props.route&&props.route.params){
                    ...
                    setProfileIndex(props.route.params['index'])
                    setData(props.route.params)
                    let list = props.route.params['data']
                    list.map((item, i) => {
                        console.log('MOBile',item.mob)
                      })
                    // setContactDetails({
                    //     ...contact,
    ...
    const getContactDetail = async (userId,partnerId,index) => {
        ...
        if(contactDetails_){
        let arrayOfPartner = [...partnerData];
        arrayOfPartner[index].mob = contactDetails_.mob;
        arrayOfPartner[index].parentContact = contactDetails_.parentContact ? contactDetails_.parentContact : 'Not Specified';
        arrayOfPartner[index].email = contactDetails_.email ? contactDetails_.email : 'Not Specified';
        await setPartnerData(arrayOfPartner)
        // await setProfileIndex(index)
        }
    }
      }
      const getDetail = async (index) => {
            console.log(index)
            await setProfileIndex(index)
      }
                            ...
                            <View style={{ width: '100%', marginTop: 5, borderRadius: 20, backgroundColor: 'white', padding: 15, alignSelf: 'center', borderWidth: 1, borderColor: "#0000001A" }}>
                                ...
                                <View>
                                    <View style={{ flexDirection: 'row' }}>
                                        ...
                                        <View style={{ marginLeft: 10, marginBottom: 10, marginTop: 10 }}>
                                            ...
                                            <Text style={{ fontSize: 14, fontWeight: '500' }}>{
                                                // contact.mob != undefined ? contact.mob : 
                                                item.mob }</Text>
                                        </View>
                                    </View>
                                    ...
                                    <View style={{ borderTopWidth: 0.5, marginTop: 15, borderColor: "#ABB2B9" }}>
                                        <TouchableOpacity
                                            onPress={() => getContactDetail(id, item.id,i)}
